    ...
    <Sidebar collapsed={!isOpen} style={{ width: "80vh" }}>
      {selectedMarkerData && (
        ...
        >
          ...
          />
          <div className="header1">{selectedMarkerData.name}</div>
          <div className="alamat">
            <MapPin className="pin" />
            {selectedMarkerData.address[0]}
          </div>
          <hr />
          ...
          <div className="container-isi">
            ...
            <div className="row2-kepala">
              <span className="warga-name">
                Nama: {selectedMarkerData.name}
              </span>
              <span className="nik">NIK: {selectedMarkerData.nik[0]}</span>
              <span className="pekerjaan">
                Pekerjaan: {selectedMarkerData.job}
              </span>
              <span className="ttl">
                Tempat Tanggal Lahir: {selectedMarkerData.ttl}
              </span>
              <span className="last-edu">
                Pendidikan Terakhir: {selectedMarkerData.lastEdu}
              </span>
              <a
                className="see-photo"
                href={selectedMarkerData.image_url_Diri}
                target="_blank"
              >
                Lihat Foto Diri
              </a>
            ...
            <div>
              {selectedMarkerData.istri ? (
                selectedMarkerData.istri.map((istriName, index) => {
                  ...
                  const endIdxIstri =
                    selectedMarkerData.nik.length - midAnakLength; // Indeks akhir untuk NIK istri

                  const nikIstri = selectedMarkerData.nik.slice(
                    startIdxIstri,
                    endIdxIstri
                  )[index];
                  const alamatIstri = selectedMarkerData.address.slice(
                    startIdxIstri,
                    endIdxIstri
                  )[index];
                  ...
                  return (
                    <div key={index} className="warga-name-container">
                      ...
                      <span className="alamat-istri">
                        {alamatIstri === selectedMarkerData.address[0]
                          ? "Alamat sama dengan Kepala Keluarga"
                          : `Alamat: ${alamatIstri}`}
                      </span>
                      {index !== selectedMarkerData.istri.length - 1 && <br />}{" "}
                      {/* Menambahkan <br> jika bukan istri terakhir */}
                    </div>
                  );
                })
              ) : (
            ...
            <div>
              {selectedMarkerData.platNomor ? (
                <div className="warga-name-container">
                  <span className="warga-name">
                    Jumlah Kendaraan: {selectedMarkerData.platNomor.length}
                  </span>
          ...
          <div className="container-isi">
            ...
            <div>
              {surat &&
              surat[selectedMarkerData.name] &&
              surat[selectedMarkerData.name].fileNames.length > 0 ? (
                surat[selectedMarkerData.name].fileNames.map(
                  (fileName, index) => (
                    <div key={index} className="warga-name-container">
                ...
                selectedMarkerData.comment.map((ket, index) => {
                  return (
                    <div key={index} className="warga-name-container">
                      ...
                      {index !== selectedMarkerData.comment.length - 1 && (
                        <br />
                      )}
                    </div>
                  );
